    ...
    internal static class ContextHelper
    {
        internal static object GetValue(this Bundle args, string key, Type type)
        {
            try
            {
                ...
                {
                    //Runtime serialization for reference type.. use json.net serialization
                ...
                else
                {
                    ...
                    throw new ArgumentException(
                        string.Format(
                            "Unsuported type. Cannot pass value to variable {0} of step {1}. Variable type is unsuported.",
                            key, type.FullName));
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine("Wizard droid error "+ex.Message+ex.StackTrace);
                return null;
            }
        }
    }
