...
namespace WizarDroid.NET
{
    ...
    public abstract class WizardFragment : Android.Support.V4.App.Fragment // Must derive from v4 fragment to support viewpager
    {
        ...
        /// <summary>
        /// Called once the fragment is associated with its context.
        /// </summary>
        ...
        ///  Typically overriden to persist the final wizard context and finish the context
        /// </summary>
        internal void OnWizardComplete()
        {
            StateManager.LoadStepContext(this);
            
            if (this.WizardCompleted != null) //Trigger the event within the fragment.. The underlying user defined wizard can add an handler to close out the context if so desired
